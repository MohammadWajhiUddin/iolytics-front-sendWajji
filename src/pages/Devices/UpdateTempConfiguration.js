...
const UpdateTempConfiguration = () => {
  ...
  const deviceTempData = useSelector(
    state => state.TempConfig.DeviceTempConfigData,
  )
  ...
  const handleSubmit = async e => {
    ...
    try {
      const response = await axios.post(
        "https://tempin.qastco.co.uk:3231/api/insight/updatedevicetempconfig",
        {
        ...
        <Col md={12}>
          <Card body>
            <h2 className="text-center mb-4 mt-4">
              Update {deviceTempData.deviceName} Device Configuration
            </h2>
            {success && (
              <Alert color="success">Data updated successfully!</Alert>
            )}
